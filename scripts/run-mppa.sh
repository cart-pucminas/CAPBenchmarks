...
for kernel in tsp; do	
	for nprocs in {1..16}; do
	    ...
	    $MPPADIR/bin/k1-power -- $MPPADIR/bin/k1-jtag-runner --multibinary=$BINDIR/$kernel.img --exec-multibin=IODDR0:master -- --verbose --class $CLASS --nclusters $nprocs &>> $RESULTSDIR/$kernel-standard-$nprocs.mppa
	done
done
