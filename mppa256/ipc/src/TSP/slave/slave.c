...
void run_tsp (int nb_threads, int nb_towns, int seed, int nclusters) {
	int i;	
	MUTEX_INIT(min_slave_to_master_lock);

	comm_buffer_size = (nclusters + 1) * sizeof(int);
	...
	broad = mppa_create_broadcast (nclusters, BROADCAST_MASK, comm_buffer, comm_buffer_size, TRUE, callback_slave);
	...
	tsp_instance = init_execution(cluster_id, nclusters, get_number_of_partitions(nclusters), nb_threads, nb_towns, seed);
...
void new_minimun_distance_found(tsp_t_pointer tsp_inst) {
	MUTEX_LOCK(min_slave_to_master_lock);
	int min_local = tsp_get_shortest_path(tsp_inst);
	mppa_broadcast (broad, &min_local, sizeof(int));	
	MUTEX_UNLOCK(min_slave_to_master_lock);
}

void callback_slave (__attribute__((unused)) mppa_sigval_t sigval) {	
	int i, min = INT_MAX;
